...
engine = create_engine('postgresql://neondb_owner:npg_dK3TSAthwBV9@ep-red-waterfall-a46bxalg-pooler.us-east-1.aws.neon.tech/neondb?sslmode=require&channel_binding=require')
...
results_local_1 = (
    _results_local
    .merge(_search_videos[["matched_term_key", "video_id", "search_run_id"]], left_on="topic_key", right_on="matched_term_key", how="inner")
...
results_int_1 = (
    _results_int.merge(_search_videos_int[['matched_term_key', 'video_id', 'search_run_id']], left_on='topic_key', right_on='matched_term_key', how='inner')
    .loc[:, ['topic', 'video_id', 'search_run_id']]
    .drop_duplicates()
)
results_int_final = (
    results_int_1.merge(videos, on="video_id", how="left")
    .loc[:, ['topic', 'video_id', 'search_run_id'] + [c for c in videos.columns if c != 'video_id']]
    .sort_values(by='published_at', ascending=False)
    .drop_duplicates(subset='video_id', keep='first')
)
...
def _pdf_build(topic, header_row, stats_dict, videos_df):
    ...
    rows = [[
        Paragraph("Thumb", header_style), Paragraph("Logo", header_style), Paragraph("Title", header_style),
        Paragraph("Channel", header_style), Paragraph("Views", header_style), Paragraph("Likes", header_style),
        Paragraph("Comments", header_style), Paragraph("Published", header_style), Paragraph("URL", header_style)
    ]]
    ...
    def _logo_from_channel_url(channel_url, max_w, max_h):
        try:
...
def render_detail_page(topic: str):
    ...
    if show.empty:
        ...
        return
    show["__is_english__"] = show["title_y"].apply(is_english_title)
    show = show[show["__is_english__"] == True]
    if is_local:
        show = show[show["title_y"].str.contains(r"\bpakistan\b", case=False, na=False) | show["title_y"].str.contains("پاکستان", case=False, na=False)]
    else:
...
if view == "report" and topic_q:
    ...
    with a1:
        ...
        for c in ["title", "channel_title", "channel_thumb", "url", "published_at", "video_id"]:
            if c not in ticker_rows_local.columns:
                ticker_rows_local[c] = ""
        ...
        for c in ["title", "channel_title", "channel_thumb", "url", "published_at", "video_id"]:
            if c not in ticker_rows_int.columns:
                ticker_rows_int[c] = ""
    ...
    with a2:
        ...
        if not country_counts.empty:
            fig = px.choropleth(
                country_counts, locations="Country", locationmode="country names", color="Videos", color_continuous_scale="Blues"
            )
